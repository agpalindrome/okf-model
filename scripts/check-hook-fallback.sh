...
#
# This repo drives its git hooks with prek rather than pre-commit for exactly
# one property: prek's template guards the store path it pins and falls back to
# PATH, where the dev shell supplies the driver. That is the whole of what #4
# bought, and #3 is what it cost when it was missing — a `git commit` failing
# as `No such file or directory` about a file that is present, naming neither
# the cause nor a way back.
#
# The property lives in a template this repo does not own and does not track:
# the hook is generated and untracked, and prek's template is upstream. So a
# release that dropped the guard would leave this repo green and silently
# un-fixed. This is what notices.
#
...
# fails this; a template that merely reworded it does not. A template that
# moved the `PREK=` line fails too, loudly, rather than passing blind.
#
# Adapted from `agpalindrome/okf-tools`, which landed the same check in its
# #100. The two repos pin the same git-hooks.nix rev (`43b3c1a`, checked
# 2026-08-18) and the same prek, so the property under test is the same one.
#
